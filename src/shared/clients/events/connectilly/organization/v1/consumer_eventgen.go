...
import (
	...
	enterpriseTime "github.com/Connectilly/connectilly/src/shared/enterprise/time"
	"go.uber.org/zap"
)
	...
	go func() {
		...
		for {
			...
			if err := json.Unmarshal(message.Payload, &event); err != nil {
				c.logger.Errorf("Failed to de-serialize OrganizationEvent message. Error: %v", err)
			...
			if err := c.subscriber.Handle(ctx, event); err != nil {
				c.logger.Errorf("Failed to handle OrganizationEvent message. Error: %v", err)
