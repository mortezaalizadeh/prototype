...
type OrganizationQueryRuleFunc func(context.Context, *entities.OrganizationQuery) error

// EvalQuery return f(ctx, q).
func (f OrganizationQueryRuleFunc) EvalQuery(ctx context.Context, q entities.Query) error {
	if q, ok := q.(*entities.OrganizationQuery); ok {
		return f(ctx, q)
	}
	return Denyf("entities/privacy: unexpected query type %T, expect *entities.OrganizationQuery", q)
}

// The OrganizationMutationRuleFunc type is an adapter to allow the use of ordinary
// functions as a mutation rule.
type OrganizationMutationRuleFunc func(context.Context, *entities.OrganizationMutation) error

// EvalMutation calls f(ctx, m).
func (f OrganizationMutationRuleFunc) EvalMutation(ctx context.Context, m entities.Mutation) error {
	if m, ok := m.(*entities.OrganizationMutation); ok {
		return f(ctx, m)
	}
	return Denyf("entities/privacy: unexpected mutation type %T, expect *entities.OrganizationMutation", m)
}
...
func mutationFilter(m entities.Mutation) (Filter, error) {
	switch m := m.(type) {
	case *entities.OrganizationMutation:
		return m.Filter(), nil
	default:
